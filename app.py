...
APP_VERSION = "v3.0-rules"
SESSION_KEY = "availability_public_v30"
...
def normalize_day(value: object) -> date:
...
if SESSION_KEY not in st.session_state:
    st.session_state[SESSION_KEY] = default_availability()

st.title("予定返信AI")
st.caption(
    "空き時間と相手のメッセージから返信候補を作ります。"
    "予定は確定せず、候補の提示まで行います。"
)
...
with left:
    ...
    edited = st.data_editor(
        ...
        hide_index=True,
        key="availability_editor_public_v30",
    )
...
if analyze:
    ...
    for index, row in edited.iterrows():
        try:
            day_value = normalize_day(row["日付"])
            start_value = row["開始"]
            end_value = row["終了"]
