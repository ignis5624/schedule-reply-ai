...
「N日後」は今日からN日後として計算してください。日付指定がなければ今日から14日後までにしてください。
...
if "availability_v21" not in st.session_state:
    st.session_state.availability_v21 = default_availability()

st.title(f"予定返信AI・試作版 {APP_VERSION}")
st.caption("N日後の解析と、連続する空き時間の一括表示に対応した版です。予定の確定は行いません。")
...
with left:
    ...
    edited = st.data_editor(
        st.session_state.availability_v21,
        num_rows="dynamic",
        ...
        hide_index=True,
        key="availability_editor_v21",
    )
    st.session_state.availability_v21 = edited

with right:
    st.subheader("2. 相手のメッセージを入力")
    message = st.text_area("受信したメッセージ", value="3日後に2時間くらい会える？", height=130)
...
st.divider()
st.caption("動作例：18:00〜22:00が空き、メッセージが『3日後に2時間』なら『18:00〜22:00の間で2時間』と表示します。")
